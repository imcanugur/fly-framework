...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .location { font-family: 'JetBrains Mono', monospace; color: var(--muted); font-size: 14px; border-left: 2px solid var(--border); padding-left: 16px; }
        ...
        .floating-actions {
            position: fixed; bottom: 40px; right: 40px; display: flex; gap: 12px;
        }
        .fab {
        ...
        <div id="env" class="pane">
            <div class="data-grid">
                <?php foreach ($_ENV as $k => $v): ?>
                    <?php if (str_contains(strtolower($k), 'pass') || str_contains(strtolower($k), 'key')) $v = '****'; ?>
                    <div class="data-row"><div class="key"><?= $k ?></div><div class="val"><?= $v ?></div></div>
                <?php endforeach; ?>
            </div>
        </div>
        <div id="cfg" class="pane">
            <pre style="font-family: 'JetBrains Mono', monospace; font-size: 13px;"><?= json_encode(config()->all(), JSON_PRETTY_PRINT) ?></pre>
        </div>
    </section>

    <div class="floating-actions">
        <div class="fab" onclick="navigator.clipboard.writeText('<?= addslashes($message) ?>')"><i data-lucide="copy" size="20"></i></div>
        <div class="fab" onclick="document.body.style.filter = document.body.style.filter ? '' : 'invert(1) hue-rotate(180deg)'"><i data-lucide="sun-moon" size="20"></i></div>
    </div>
